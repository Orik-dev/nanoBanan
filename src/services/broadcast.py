...
async def broadcast_send(ctx: dict[str, Any], job_id: str):
    """
    ✅ ИСПРАВЛЕНО: Production-ready рассылка с защитой от зависания и race conditions
    """
    bot: Bot = ctx["bot"]
    ...
    async with SessionLocal() as session:
        ...
        # ✅ ИСПРАВЛЕНО: транзакция с блокировкой
        ...
        async def _send(chat_id: int, text: str, media_type: str | None, 
            ...
            # ✅ ИСПРАВЛЕНО: таймаут на получение токена (защита от deadlock)
            try:
        ...
        while not cancelled:
            # ✅ ИСПРАВЛЕНО: проверка отмены В ТРАНЗАКЦИИ с блокировкой
            async with session.begin():
                st_row = await session.execute(
                    select(BroadcastJob.status)
                    .where(BroadcastJob.id == job_id)
                    .with_for_update()  # ✅ Блокируем запись
                )
                status = st_row.scalar_one_or_none()
